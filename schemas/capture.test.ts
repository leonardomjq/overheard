...
describe("TweetDataSchema", () => {
  ...
  it("rejects invalid datetime format", () => {
    const bad = { ...rawCapture.tweets[0], timestamp: "not-a-date" };
    expect(() => TweetDataSchema.parse(bad)).toThrow();
  });

  it("rejects invalid media URLs", () => {
    const bad = { ...rawCapture.tweets[0], media_urls: ["not-a-url"] };
    expect(() => TweetDataSchema.parse(bad)).toThrow();
  });
});
...
describe("RawCaptureSchema", () => {
  ...
  it("rejects capture with more than 500 tweets", () => {
    const tweet = rawCapture.tweets[0];
    const bad = {
      ...rawCapture,
      tweets: Array(501).fill(tweet),
    };
